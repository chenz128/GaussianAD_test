...
mkdir -p "${WORK_DIR}"
{
  echo "=== v15 safety-calibrated residual DDIM launch $(date '+%Y-%m-%d %H:%M:%S') ==="
  echo "git branch : $(git rev-parse --abbrev-ref HEAD)"
  echo "git commit : $(git rev-parse HEAD)"
  echo "config     : ${CONFIG}"
  echo "python     : ${PYTHON}"
  echo "generator  : v14 residual DDIM architecture + v15 safety head"
  echo "init       : ${VERIFIED_V12_CHECKPOINT} (v12-fixempty, weights only)"
  echo "work dir   : ${WORK_DIR}"
  echo "DDIM       : K=4, NFE=${DDIM_STEPS:-4}, fixed noise, no CFG"
  echo "safety     : metric-aligned vehicle SAT calibration + safety-first CVaR"
