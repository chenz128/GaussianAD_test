"""
render_focus 实验配置
目标：验证 2D 渲染监督（RenderLoss）对高斯语义和深度的有效性
GPU：3 张（5, 6, 7）
差异于 splatting 主配置：
  - 移除 MapLoss、PlanLoss
  - sem_lw 2.0 → 5.0，depth_lw 0.05 → 0.5（强化 2D render 梯度信号）
  - frozen_modules: ['map_decoder', 'planner_head']（节省显存和计算）
  - history_no_grad=True（继承 splatting 加速方案）
"""
...
# ========= loss config ================
# render_focus：去掉 Map+Plan，拉高 RenderLoss 内部权重以测试 2D 监督有效性
# Loss 量级对比（预期，相对 splatting）：
#   OccupancyLoss:     ~5    (不变)
#   OccupancyFlowLoss: ~15   (不变)
#   DetectionLoss:     ~2    (不变)
#   RenderLoss:        ~5-9  (sem_lw×CE_sem + depth_lw×Huber_depth，从~2 提升到~7)
loss = dict(
    type='MultiLoss',
    loss_cfgs=[
        dict(
        ...
        # [render_focus] 核心实验：强化 2D 渲染监督
        # sem_lw=5.0（原 2.0），depth_lw=0.5（原 0.05）
        # 语义 CE 贡献从 ~1.5 → ~3.5-7.5，深度 Huber 贡献从 ~0.1 → ~1-2.5
        dict(
            type='RenderLoss',
            weight=1.0,
            sem_lw=5.0,
            depth_lw=0.5,
            vis_dir='out/nuscenes_gs25600_render_focus/render_vis',
            vis_every=500,
        ),
    ])
...
loss_input_convertion = dict(
    ...
    # render loss inputs（2D pseudo-label supervision）
...
# map_decoder 和 planner_head 冻结，节省显存和计算（对应 loss 已移除）
frozen_modules = ['map_decoder', 'planner_head']
find_unused_parameters = False
backbone_fp16 = True  # backbone+neck 使用 bf16，其余 fp32
...
train_dataset_config = dict(
    ...
    pc_range=pc_range,
    num_frames=4,
    # pseudo-label 配置（render_h=256=900×0.44-140, render_w=704=1600×0.44）
...
model = dict(
    ...
    history_no_grad=True,    # 历史帧 backbone+encoder 不计算梯度，节省显存
    img_backbone=dict(
        _delete_=True,
        type='ResNet',
        depth=101,
    ...
    head=dict(
        ...
        cuda_kwargs=dict(
            ...
            grid_size=0.5),
        # 2D 渲染配置：render_h=256（900×0.44-140），render_w=704（1600×0.44）
        # sem_lw/depth_lw 需与 RenderLoss 配置一致
        render_config=dict(
            render_h=256,
            render_w=704,
            sem_lw=5.0,
