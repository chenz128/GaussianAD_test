"""
nuscenes_gs25600_2D 实验配置
目标：验证 2D 渲染监督（RenderLoss）对高斯语义和深度的有效性
GPU：3 张（5, 6, 7）
差异于 v4test（本地 nuscenes_gs25600.py）：
  - 移除 MapLoss、PlanLoss
  - 新增 RenderLoss（sem_lw=5.0, depth_lw=0.5），分开显示 RenderSemLoss/RenderDepthLoss
  - frozen_modules: ['map_decoder', 'planner_head']
其他配置与 v4test 一致：
  - history_no_grad=False
  - v4 PKL
  - backbone_fp16=True, img_backbone with_cp=True
"""
