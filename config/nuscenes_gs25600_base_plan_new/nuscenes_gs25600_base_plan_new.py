"""
nuscenes_gs25600_base_plan_new —— base_gt_ego_fixempty 基础上接入 planner 闭环

【背景】
base_gt_ego_fixempty（current 15.10 / FutAvg 8.33）是修复 empty-gaussian bug
后的合法基线（flow_grad_scale=1.0，无 Dyn/Phys，frozen map/plan）。
本实验在此之上做"部署闭环"对比：
...
（从 epoch 0 计数训 15 轮，optimizer/lr 全新，适配新解冻的参数）。
不要 --resume-from（否则 optimizer state 不匹配，train.py 会自动跳过）。

GPU：h20-new 后 4 张（4,5,6,7）。启动见同目录 train.sh。
"""
_dim_ = 256
_pos_dim_ = 128
...
load_from = 'out/nuscenes_gs25600_base_gt_ego_fixempty/checkpoints/epoch_15.pth'
loss = dict(
    loss_cfgs=[
        dict(
...
work_dir = 'out/nuscenes_gs25600_base_plan_new'
xyz_coordinate = 'cartesian'
