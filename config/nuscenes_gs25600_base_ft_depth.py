"""
nuscenes_gs25600_base_ft_depth 实验配置（base epoch5 接续 + 多帧深度微调）
目标：验证“在已训好的 base（occ+flow+det，epoch5，mIoU≈8.8）基础上接续，加入
      当前帧 + 历史 2 帧 + 未来 2 帧 的 2D 深度渲染监督”是否能带来增益。
      与从零训的 mf_depth 对比，回答“好初始化 + depth 是否有效”。
关键设置：
  ...
  - val num_samples=2000, subsample_seed=42（与 base 同一 val 子集，可直接对比）
  - 移除 MapLoss、PlanLoss，frozen_modules: ['map_decoder', 'planner_head']
"""
...
include_opa = True
load_from = 'ckpts/base_epoch_5.pth'  # base（occ+flow+det）第 5 epoch 权重，strict=False 只加载权重，epoch 从 0 重新计数
semantics = True
semantic_dim = 17
...
train_dataset_config = dict(
    ...
    pc_range=pc_range,
    num_frames=4,
    # 轻量验证：子采样 3000 个 keyframe，固定 seed 保证可复现
    num_samples=3000,
    subsample_seed=42,
    # 多帧深度监督：历史 2 帧 + 未来 2 帧
    num_hist_depth_frames=2,
    num_fut_depth_frames=2,
    # pseudo-label
