...
【为什么要重跑 base】
原 base 训练于 6/16-6/23，当时 forward_flow 的 ego 补偿取自 `ego_fut_preds`，
而 planner_head 是冻结的随机初始化 → 补偿值几乎为 0，OccFlowLoss 只能逼 offset
自己去编码 -ego位移（这正是 base 的 offset 出现"全局 drift"的原因）。
commit 3a7ec54(7/10) 之后 forward_flow 改用 GT `ego_fut_trajs`，oracle v4 及以后
全部使用新公式。因此 base vs oracle 的未来帧指标此前不可比：
  - base15(7/1 测)：训练与测试都无 ego 补偿 -> 自洽，但属于另一套公式
  - base30(7/14 测)：训练无、测试有 -> 双重补偿，数字被污染
本实验用当前代码重跑 base，使其与 v10/v12 处于同一套 ego 补偿公式下，
从而给出可信的 current / future 目标线。

【与原 base 的唯一差异】
代码版本（GT ego 补偿）。loss 组成、数据子集(3000/2000, seed=42)、冻结模块
(map_decoder + planner_head)、backbone / 预训练权重全部保持不变。
max_epochs 恢复为 15：原 base 最初即为 15（30 是后来为接续训练所改），
base15=14.15 正是 15 轮调度跑完、学习率完整退火的结果，与 oracle 系列同调度。

【保持 base 语义（以下均为代码默认值，不显式设置）】
  - 无 DynamicLoss / PhysicsLoss，仅 occ + flow + det
  - refine_layer.use_dynamic=False、decouple_offset=False（offset 不解耦）
  - head.flow_grad_scale=1.0（未来帧梯度照常回传当前高斯 = 旧行为）

【用途】给出 v12 真正该对标的 current mIoU 目标线，以及同公式下公平的 future 起点。

GPU：h20-old 后 4 张（4,5,6,7）。启动见同目录 train.sh。
"""
_dim_ = 256
_pos_dim_ = 128
...
work_dir = 'out/nuscenes_gs25600_base_gt_ego'
xyz_coordinate = 'cartesian'
