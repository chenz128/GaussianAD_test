"""
nuscenes_gs25600_occdet_light 实验配置（轻量化快速迭代版）
目标：从头训练（只留 backbone 预训练），**仅保留 occ + flow + detection 三项监督**，
      去掉 map / plan / render / dynamic 所有其他监督，作为最小监督 baseline。
GPU：h20-new 后 4 张（4, 5, 6, 7）
轻量化（与 dynamic_light 一致）：
  - train_dataset_config.num_samples=3000 训练集子采样（全量 ~28k → 3000）
  - subsample_seed=42 固定，baseline/实验复用同一子集，结果可比、可复现
监督配置：
  - OccupancyLoss + OccupancyFlowLoss + DetectionLoss
  - 无 RenderLoss / DynamicLoss / MapLoss / PlanLoss
  - dataset 不加载任何伪标签（不传 metric3d/grounded_sam/dynamic_gt root），加速数据读取
  - 模型关闭 2D 渲染（render_config=None）与 dynamic head（use_dynamic=False），省显存/算力
...
frozen_modules = ['map_decoder', 'planner_head']
# No dynamic head / render path -> unused params come from flow layers
...
train_dataset_config = dict(
    ...
    # No pseudo-label roots passed -> use_pseudo_label=False -> no metric3d/sam/dynamic
    # GT loading and no multi-frame assembly (occ+flow+det supervision only).
    num_samples=3000,
    subsample_seed=42,
)
...
model = dict(
    ...
    head=dict(
        ...
        cuda_kwargs=dict(
            ...
            grid_size=0.5),
        # render_config=None -> no 2D splatting (no render/dynamic supervision)
    ),
    decoder=dict(
        type='VoxelNeXt',
        num_class=10,
