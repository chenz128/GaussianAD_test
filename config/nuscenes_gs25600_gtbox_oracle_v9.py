"""
nuscenes_gs25600_gtbox_oracle_v9 实验配置
v9 = v8 的权重基础 + kinematic (CTRV) offset 头 → 让 offset 学会拐弯。

【与 v8 的唯一自变量：offset 参数化方式】
v8（free）：offset 头直接输出 6×2=12 个自由数 → 稀疏监督下塔成直线。
v9（kinematic）：offset 头输出每高斯 3 个运动学参数 [vx, vy, omega]，经可微 CTRV
（恒转向率恒速）rollout 积分成同样的 (6, 2) 累计位移；输出天然是圆弧（omega≠0
即拐弯），不可能塔成退化直线，且只需拟合 3 个低方差标量，稀疏 GT 下更易学到转向。
下游（forward_flow / PhysicsLoss）无需改动，仍接收 (6, 2)。

【初始化：暖启 v8】
kinematic 头输出维从 12 变为 3，offset_layers 形状与 v8 不兼容，load_from 以 strict=False
加载：编码器 / occ 头 / det 头全部暖启（当前帧从第 0 步就 == v8），只有 offset_layers
随机重 init，只需把新的 kinematic 头训起来 → 最快验证“能不能拐弯”。
⚙ 前置：先跑完 v8 并确认 offset 真的“动起来”（offset_dyn_rms 上升、loss_traj 下降），
  再把下面 load_from 指向 v8 的最优 checkpoint。

其余（loss 权重 static_w=1/traj_w=4/rigid_w=1/beta=1/pos_weight=8、decouple_offset=True、
offset_grad_scale=0.0、GT-box oracle 门控、3000 子集、冻结 map/plan）与 v8 完全相同。
...
loss = dict(
    ...
    loss_cfgs=[
        ...
        dict(
            ...
            sem_lw=5.0,
            depth_lw=0.5,
            vis_dir='out/nuscenes_gs25600_gtbox_oracle_v9/render_vis',
            vis_every=500,
        ),
        dict(
            type='DynamicLoss',
            ...
            vis_dir='out/nuscenes_gs25600_gtbox_oracle_v9/dynamic_vis',
            vis_every=500,
        ),
        dict(
            type='PhysicsLoss',
            weight=1.0,
            static_w=1.0,
            # v7: loss_smooth OFF (smooth_w=0). It penalized ALL acceleration
            # (forced constant velocity), conflicting with real braking /
            # acceleration / turning now supervised by loss_traj.
            smooth_w=0.0,
            rigid_w=1.0,
            # v7: loss_vel OFF (constant-velocity straight-line v_box*t),
            # replaced by loss_traj using the instance's REAL GT future
            # trajectory (gt_agent_fut_trajs cumulative). traj_w drives motion.
            vel_w=0.0,
            traj_w=4.0,
            # v8: keep beta=1.0 (neutral). SmoothL1 grad saturates to +/-1 for
            # |err|>beta anyway; real dynamic displacements are >beta, so a
            # larger beta only shrinks the near-region gradient and SLOWS the
            # "make gaussians move" objective. traj_w (not beta) is the lever.
...
loss_input_convertion = dict(
    ...
    # v7: per-instance GT future trajectory (aligned to gt_boxes rows) for
    # loss_traj. Pulled from metas (not in result_dict).
...
include_opa = True
# v9: run IN PARALLEL with v8 -> cannot warm-start from v8's (unfinished)
# checkpoint, so start from the same r101 pretrain as v8. This is the cleaner
# same-init A/B anyway (v8 free-offset vs v9 kinematic-offset from identical
# start). The kinematic head is trained from scratch; current frame re-converges
# alongside v8. (To instead warm-start after v8 finishes, swap to the v8 line.)
load_from = 'ckpts/r101_dcn_fcos3d_pretrain.pth'
# load_from = 'out/nuscenes_gs25600_gtbox_oracle_v8/latest.pth'
semantics = True
semantic_dim = 17
...
model = dict(
    ...
    temporal_encoder=dict(
        ...
        refine_layer=dict(
            ...
            semantics_activation='softplus',
            use_dynamic=True,
            # v6: decouple offset from the encoder. The offset head reads a
            # detached feature, so PhysicsLoss (static/smooth/rigid/vel) and
            # OccFlowLoss-via-offset gradients train only the offset head and
            # never pollute the occupancy encoder -> current-frame occ recovers
            # to base level while offset still learns motion.
            decouple_offset=True,
            # v8: keep FULL detach (offset_grad_scale=0.0) so the current frame
            # stays == v7. The gradient leak is the only current-frame risk.
            offset_grad_scale=0.0,
            # v9: kinematic (CTRV) offset. The head predicts 3 params
            # [vx, vy, omega] per gaussian; _kinematic_rollout integrates them
            # into the same (6, 2) cumulative displacement as an arc, so the
            # offset can TURN (omega!=0). Requires decouple_offset=True.
