"""
nuscenes_gs25600_depth_init 实验配置（方案 A：Metric3D 深度伪标签初始化高斯位置）

目标：验证用 Metric3D 深度图反投影的 3D 点来初始化每帧的高斯位置，
      是否比固定随机初始化（base）带来 mIoU 增益。
关键设置：
  - lifter: pts_init=True, anchor_grad=True（非xyz属性可学习）
  - dataset: depth_init_root 指向 Metric3D 深度图目录
  - 不使用 RenderLoss，不使用 pseudo_seg，只用 depth 做初始化
  - 其余 loss（occ+flow+det+map+plan）与 base 完全一致
  - max_epochs=15，从零训练
  - num_samples=3000, subsample_seed=42（与其他实验同子集）
...
# depth init root (Metric3D pseudo depth on H20)
_depth_init_root = '/data/chenz/Gaussianflowocc_test/data/metric_3d_nusc'
...
val_dataset_config = dict(
    ...
    num_samples=2000,
    subsample_seed=42,
    depth_init_root=_depth_init_root,
)
