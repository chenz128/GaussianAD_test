"""
nuscenes_gs25600_gtbox_oracle_v7 实验配置
v7 = v6（offset 解耦 decouple_offset=True）+ 精细化物理约束：
让 offset 贴近真实物理运动，而非被恒速直线先验压平。
相对 v6 的改动（仅 PhysicsLoss，其余完全一致）：
  - loss_vel（恒速直线外推 v_box*t）→ loss_traj（GT 实例真实未来轨迹）：
      vel_w=1.0 → 0.0，新增 traj_w=1.0。
      轨迹来自 attr_labels_planner（gt_agent_fut_trajs 逐步 delta 累加），
      与 gt_boxes 行对齐、同为 LIDAR 系纯物体位移（无 ego）。
      → 加速/刹车/转弯/变道均可被监督，不再强制匀速直线。
  - loss_smooth（惩罚所有加速度=强制匀速）关闭：smooth_w=20.0 → 0.0。
  - loss_rigid 保持不变（rigid_w=10.0），loss_static 保持（static_w=2.0）。
  - loss_input_convertion 新增 attr_labels_planner（从 metas 流入）。
其余（GT-box oracle 动静门控、2D 渲染关闭、伪标签关闭、3000 子集、冻结 map/plan、
decouple_offset=True）与 v6 完全相同，保证只动“恒速直线→GT 真实轨迹”一个变量。
...
loss = dict(
    ...
    loss_cfgs=[
        ...
        dict(
            ...
            sem_lw=5.0,
            depth_lw=0.5,
            vis_dir='out/nuscenes_gs25600_gtbox_oracle_v7/render_vis',
            vis_every=500,
        ),
        dict(
            type='DynamicLoss',
            ...
            vis_dir='out/nuscenes_gs25600_gtbox_oracle_v7/dynamic_vis',
            vis_every=500,
        ),
        dict(
            type='PhysicsLoss',
