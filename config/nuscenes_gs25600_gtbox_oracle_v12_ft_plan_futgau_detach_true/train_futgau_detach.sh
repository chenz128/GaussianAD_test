#!/usr/bin/env bash
# ============================================================================
# v12_fixempty_ft_plan_futgau_detach —— 未来帧高斯融合 planner 实验 (plan_ego_detach=True)
#
...
#
# plan_ego_detach=True：occ_flow 未来帧 ego 补偿 (means_fut - planner_res) 处
#   切断梯度，OccFlowLoss 不再回传 planner，避免一致性损失污染 planner 训练。
#
# 对照实验见同目录 train_futgau.sh (plan_ego_detach=False)。
#
# GPU：0,1,2,3   端口：12485
# 用法：tmux new -s train_futgau_detach
#       bash config/nuscenes_gs25600_gtbox_oracle_v12_ft_plan_futgau/train_futgau_detach.sh
# 监控：epoch 2 起 planner 预测生效，对比 train_futgau.sh 的 plan_L2 / obj_box_col。
# ============================================================================
set -euo pipefail
...

{
  ...
  echo "plan_ego_detach: True (切断 occ_flow->planner 梯度)"
