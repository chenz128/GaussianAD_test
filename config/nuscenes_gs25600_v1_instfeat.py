"""
nuscenes_gs25600_v1_instfeat 实验配置（GaussianFormer V1：独立可学习 instance feature）

目标：与 base（随机初始化）对照，验证 GaussianFormer V1 的核心改动——
      instance feature（query）从"由 anchor 几何派生"改为"独立可学习 nn.Parameter"。
关键设置（与 base 完全对齐，仅 lifter/encoder 的 instance feature 来源不同）：
  - lifter: 随机初始化 xyz（去掉 init_xyz_path），feat_grad=True 启用独立
            可学习 instance_feature，forward 返回 rep_features
  - encoder: 用 rep_features 作为初始 instance_feature（而非 anchor_encoder(anchor)）
  - 只用 occ+flow+det 三个 loss，不用 map/plan（与 base 一致）
  - max_epochs=15，从零训练
  - num_samples=3000, subsample_seed=42（与 base 同子集、同种子）
