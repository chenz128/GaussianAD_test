"""
nuscenes_gs25600_gtbox_oracle_v8 实验配置
目标：稳住当前帧（== v7）、改善未来帧、让动态 offset 真正“动起来”且处于合理量级
（本次不追求转向，只要位移幅度合理）。

【关键：为什么这些改动不会碰当前帧】
offset 是 decouple_offset=True（offset 头读 feat.detach()），且当前帧 occ 用 means（不用
offset，offset 只用于 forward_flow 的未来帧）。因此下面四个权重的梯度只进
offset_layers，数学上不改变编码器梯度 → 当前帧 ≈ v7。

相对 v7 的改动（均位于解耦的 offset 头，零当前帧风险）：
  1. PhysicsLoss.static_w 2.0 → 1.0（减半静态拑制；不过度，保护未来帧静态类不飘）；
  2. PhysicsLoss.traj_w  1.0 → 4.0（真正的放大杠杆：SmoothL1 梯度被 ±1 截断，
     靠 traj_w 抬高每步有效步长，让动态高斯在共享头上胜过持续的 static 拉力）；
  3. PhysicsLoss.traj_smooth_beta = 1.0（保持中性：真实位移多 >β，增大 β 反而拖慢，
     对“动起来”无帮助）；
  4. PhysicsLoss.rigid_w 10.0 → 1.0（避免 rigid 在零位移处变成第二个 static）。

【故意保持与 v7 一致、不动的旋钮（它们会碰编码器/当前帧）】
  - DynamicLoss.pos_weight 保持 8.0：dynamic_logits 由 dynamic_layers(feat) 产出（feat 未
    detach），提高 pos_weight 会加强回流编码器的梯度 → 扰动当前帧。且 oracle 模式下
    门控用 GT box，dynamic_logits 不影响 offset 训练，与本次目标正交 → 不动。
  - offset_grad_scale 保持 0.0（全解耦）：泄漏梯度才是真正的当前帧风险点。
    若 v8 跑完 offset 仍不动，再用 v8b（仅加 offset_grad_scale=0.05）验证“特征是否缺
    朝向信息”。因 offset 来自看了 4 帧的 temporal encoder，特征很可能已含运动线索，
    所以先跑零风险的 v8。

【数据隐患】训练日志中很多 iter dyn=0（运动梯度间歇缺失）。若 loss_traj 仍不稳，
考虑把 num_samples 提到 6000 或对含动态目标的场景加权采样（本配置保持 3000 以与 v7 可比）。

其余（GT-box oracle 动静门控、2D 渲染关、伪标签关、3000 子集、冻结 map/plan、decouple_offset=True）
与 v7 相同。
...
loss = dict(
    ...
    loss_cfgs=[
        ...
        dict(
            ...
            sem_lw=5.0,
            depth_lw=0.5,
            vis_dir='out/nuscenes_gs25600_gtbox_oracle_v8b/render_vis',
            vis_every=500,
        ),
        dict(
            type='DynamicLoss',
            ...
            vis_dir='out/nuscenes_gs25600_gtbox_oracle_v8b/dynamic_vis',
            vis_every=500,
        ),
        dict(
            type='PhysicsLoss',
...
model = dict(
    ...
    temporal_encoder=dict(
        ...
        refine_layer=dict(
            ...
            # v8b: leak 5% of the offset-head gradient into the shared encoder
            # feature so it can encode a little motion-direction info. Run this
            # ONLY if v8 (full detach) leaves offset collapsed. This is the sole
            # current-frame-occ risk knob; A/B its t=0 mIoU against v8.
            offset_grad_scale=0.05,
        ),
        spconv_layer=dict(
            type='SparseConv4D',
            in_channels=embed_dims,
