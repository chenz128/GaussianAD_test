    ...
    with torch.no_grad():
        for i_iter_val, data in enumerate(val_dataset_loader):
            ...
            for idx, pred in enumerate(result_dict['pred_occ'][-1]):
                ...
                # pred_scores = result_dict['pred_scores']
                # pred_labels = result_dict['pred_labels']
                # gt_boxes = result_dict['gt_boxes']
                # os.makedirs(os.path.join(args.work_dir, 'vis'), exist_ok=True)
                # pred_file = os.path.join(args.work_dir, 'vis',f'val_{i_iter_val}_pred.npz')
                # save_dict = {
                ...
                # }
                # np.savez_compressed(pred_file, data=save_dict)
