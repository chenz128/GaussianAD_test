...
const trackOcc=DATA.tracks.map(()=>[]),backgroundOcc=[];
function inFirstBox(v,t){const f=t.frames[0],c=Math.cos(f.yaw),s=Math.sin(f.yaw),dx=v[0]-f.x,dy=v[1]-f.y,dz=v[2]-f.z;const lx=c*dx+s*dy,ly=-s*dx+c*dy;if(Math.abs(lx)>t.dx/2||Math.abs(ly)>t.dy/2||Math.abs(dz)>t.dz/2)return null;return [lx,ly,dz,v[3]]}
DATA.occ.forEach(v=>{let local=null;for(let i=0;i<DATA.tracks.length;i++){local=inFirstBox(v,DATA.tracks[i]);if(local){trackOcc[i].push(local);break}}if(!local)backgroundOcc.push(v)});
function makePoints(rows,size=.35){if(!rows.length)return null;const p=new Float32Array(rows.length*3),c=new Float32Array(rows.length*3);rows.forEach((v,i)=>{p.set(v.slice(0,3),i*3);const q=PAL[v[3]]||[128,128,128];c.set(q.map(x=>x/255),i*3)});const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(p,3));g.setAttribute('color',new THREE.BufferAttribute(c,3));return new THREE.Points(g,new THREE.PointsMaterial({size,vertexColors:true,sizeAttenuation:true}))}
let occ=makePoints(backgroundOcc);if(occ)scene.add(occ);
