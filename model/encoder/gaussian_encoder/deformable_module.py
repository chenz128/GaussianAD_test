...
class SparseGaussian3DKeyPointsGenerator(BaseModule):
    ...
    def forward(
        ...
        if self.num_learnable_pts > 0 and instance_feature is not None:
            learnable_scale = (
                safe_sigmoid(self.learnable_fc(instance_feature).reshape(bs, num_anchor, self.num_learnable_pts, 3)) * 2 - 1
...
class DeformableFeatureAggregation(BaseModule):
    ...
    def project_points(key_points, projection_mat, image_wh=None):
        ...
        projection_mat = projection_mat.reshape(bs, -1, 4, 4)
        image_wh = image_wh.reshape(bs, -1, 2)
