...
class SparseGaussian3DEncoder(BaseModule):
    def __init__(
        ...
        if offset:
            self.offset_fc = embedding_layer(offset_dim)
            self.offset_start = self.semantic_start + self.semantic_dim
            self.offset_dim = offset_dim
    ...
    def forward(self, box_3d: torch.Tensor):
        ...
        if self.offset:
            offset_feat = self.offset_fc(box_3d[..., self.offset_start: (self.offset_start + self.offset_dim)])

        output = xyz_feat + scale_feat + rot_feat + opacity_feat + semantic_feat + offset_feat
