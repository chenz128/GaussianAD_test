...
from loss import MAPTR_LOSS
from mmengine import build_from_cfg
from ..coders.nms_free_coder import MapTRNMSFreeCoder
...
class MapTRv2Head(nn.Module):
    ...
    def __init__(self,
        ...
        self.z_cfg = z_cfg
        self.map_adapter_output_channels = 256
        self.embed_dims = self.map_adapter_output_channels
        ...
        self.loss_seg = MAPTR_LOSS.build(loss_seg)
        self.loss_pv_seg = MAPTR_LOSS.build(loss_pv_seg)
        ...
        # map adapter
        map_adapter_list = []
        for _ in range(num_map_adapter_conv- 1):
            map_adapter_list.append(spconv.SparseSequential(
                spconv.SubMConv2d(map_adapter_input_channels, map_adapter_input_channels, map_adapter_kernel_size, padding=int(map_adapter_kernel_size//2), bias=map_adapter_use_bias, indice_key='map_adapter'),
                nn.BatchNorm1d(map_adapter_input_channels),
                nn.ReLU()
            ))
        map_adapter_list.append(spconv.SubMConv2d(map_adapter_input_channels, self.map_adapter_output_channels, 1, bias=True, indice_key='map_adapter_out'))
        self.map_adapter_layers = nn.Sequential(*map_adapter_list)
    ...
    def get_map_inputs(self, hs):
        device = hs.indices.device
        pc_range_tensor = torch.tensor(self.pc_range, device=device)
        spatial_shape_tensor = torch.tensor(hs.spatial_shape, device=device)
        references = (hs.indices[:, 1:] + 0.5) / spatial_shape_tensor
        references = references.repeat(1, 20).unsqueeze(0)  # [bs, num_vec, 40]

        hs = self.map_adapter_layers(hs).features.unsqueeze(0) # [bs, num_vec, dim]
    ...
    def forward(self, hs):
        hs, reference = self.get_map_inputs(hs)
        ...
        # reference = reference[...,0:2] if not self.z_cfg['gt_z_flag'] else reference[...,0:3]
        reference = inverse_sigmoid(reference)

        # vec_embedding = hs[lvl].reshape(bs, self.num_vec, -1)
        outputs_class = self.cls_branches(hs)
        tmp = self.reg_branches(hs)
        ...
        outs = {
            # 'bev_embed': bev_embed,
            'all_cls_scores': outputs_class.unsqueeze(0),
            'all_bbox_preds': outputs_coord.unsqueeze(0),
            'all_pts_preds': outputs_pts_coord.unsqueeze(0),
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'enc_pts_preds': None,
            'depth': None,
