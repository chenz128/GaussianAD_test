    ...
    def __init__(
        ...
        **kwargs,
        ...
        if temporal_encoder is not None:
            self.temporal_encoder = builder.build_head(temporal_encoder)
        if head is not None:
            self.head = builder.build_head(head)
        if decoder is not None:
            self.decoder = builder.build_head(decoder)
        if map_decoder is not None:
            self.map_decoder = builder.build_head(map_decoder)
        if planner_head is not None:
            self.planner_head = builder.build_head(planner_head)
