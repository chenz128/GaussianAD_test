...
def get_cls_results(gen_results,
    ...
    if num_res > 0:
        ...
        cls_scores = np.array([score.cpu().detach().numpy() for score in cls_scores])[:,np.newaxis]
...
def format_res_gt_by_classes(gen_results,
    ...
    print('Cls data formatting done in {:2f}s!! with {}'.format(float(time.time()-timer),formatting_file))
    pool.close()
    return cls_gens, cls_gts
...
def eval_map(gen_results,
    ...
    for i, clsname in enumerate(cls_names):
        ...
        print('cls:{} done in {:2f}s!!'.format(clsname,float(time.time()-timer)))
    pool.close()
    aps = []
